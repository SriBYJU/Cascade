...
class RouteFeatures(StrictModel):
    step_type:StepType; task_text:str; risk:RiskLevel=RiskLevel.LOW; write_intent:bool=False; relevant_files:int=Field(default=0,ge=0); predicted_write_files:list[str]=Field(default_factory=list); has_strong_validation:bool=False; has_tests:bool=False; security_sensitive:bool=False; data_migration:bool=False; ambiguity:bool=False; deterministic_candidate:bool=False; prompt_cache_affinity:float=Field(default=0.0,ge=0.0,le=1.0); remaining_budget_ratio:float=Field(default=1.0,ge=0.0,le=1.0); quota_pressure:float=Field(default=0.0,ge=0.0,le=1.0); context_tokens_estimate:int=Field(default=0,ge=0); historical_success:dict[str,float]=Field(default_factory=dict)
